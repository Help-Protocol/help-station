...
import { KARMAPriceQuery } from "../contract/normalize"
...
const assetsAPRQuery = selector({
  ...
  get: async ({ get }) => {
    ...
    const karmaPrice = get(KARMAPriceQuery)
    ...
    const getAPR = (token: string) => {
      ...
      if (annualReward === "0" || karmaPrice === "0") {
